...
patch(Navbar.prototype, {
    ...
    _addPricelistButtons() {
        ...
        wrapper.className = 'pos-pricelist-buttons align-items-center flex-shrink-0';
    ...
    _isInOrder() {
        // Only show buttons on ProductScreen (Register / inside an order)
        // Hide on FloorScreen (tables), TicketScreen (orders), PaymentScreen, etc.

        // Method 1: Check URL - ProductScreen uses /product path
        const url = window.location.pathname || '';
        if (url.includes('/floor') || url.includes('/ticket')) {
            console.log("URL indicates floor/ticket screen, hiding buttons");
            return false;
        }
        if (url.includes('/product')) {
            console.log("URL indicates ProductScreen, showing buttons");
            return true;
        }

        // Method 2: Try router state (various Odoo versions)
        try {
            const currentRoute = this.pos?.router?.state?.current
                || this.pos?.router?.activeSlot
                || this.pos?.mainScreen?.component?.name;
            console.log("Current POS route:", currentRoute);
            if (currentRoute) {
                const routeName = String(currentRoute).toLowerCase();
                if (routeName.includes('product')) return true;
                if (routeName.includes('floor') || routeName.includes('ticket') || routeName.includes('payment')) return false;
            }
        } catch (e) {
            console.log("Router state check failed:", e);
        }

        // Method 3: DOM-based detection as fallback
        // FloorScreen has .floor-map or table elements
        if (document.querySelector('.floor-map, .table-container, .floor-selector')) {
            console.log("DOM indicates FloorScreen, hiding buttons");
            return false;
        }
        // TicketScreen has .ticket-screen or orders list
        if (document.querySelector('.ticket-screen, .orders-list-container')) {
            console.log("DOM indicates TicketScreen, hiding buttons");
            return false;
        }
        // PaymentScreen
        if (document.querySelector('.payment-screen, .paymentlines')) {
            console.log("DOM indicates PaymentScreen, hiding buttons");
            return false;
        }
        // ProductScreen has product list / category buttons
        if (document.querySelector('.product-list, .product-screen, .category-list')) {
            console.log("DOM indicates ProductScreen, showing buttons");
            return true;
        }

        console.log("Could not determine screen, defaulting to hide");
        return false;
    },
    ...
    _updateVisibility() {
        ...
        // Show buttons only when inside an order (ProductScreen)
